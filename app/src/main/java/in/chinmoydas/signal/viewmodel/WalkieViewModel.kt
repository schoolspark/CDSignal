...
class WalkieViewModel(private val repository: MainRepository) : ViewModel() {
    ...
    var isBroadcastMode by mutableStateOf(false)
        private set
    var isSilenced by mutableStateOf(false)
        private set
    ...
    fun saveInternetContact(name: String, code: String, onSuccess: () -> Unit, onError: () -> Unit) {
        ...
        viewModelScope.launch {
            ...
            if (token.isBlank() || token == "OFFLINE_TOKEN") {
                _uiState.value = UiState.Error("Need Internet")
                onError()
                return@launch
            }
            try {
                ...
                _uiState.value = UiState.Error("Link Error")
                onError()
            }
        }
    }
    ...
    fun startTransmission(onIpsFound: (List<String>, Int) -> Unit, onUpdateIps: (List<String>) -> Unit) {
        ...
        if (isBroadcastMode) {
            ...
            if (allIps.isNotEmpty()) {
                ...
                _uiState.value = UiState.Error("No Neighbors")
            }
            return
        }

        if (targetUser.isEmpty()) {
            _uiState.value = UiState.Error("No Target Selected")
            return
        }
        ...
        val token = repository.getToken() ?: ""
        if (token.isBlank() || token == "OFFLINE_TOKEN") {
            _uiState.value = UiState.Error("Offline")
            return
        }
        ...
        if (targetUser.startsWith("group:", ignoreCase = true)) {
            ...
            viewModelScope.launch {
                try {
                    ...
                    // FIX: Collect BOTH Public and Local IPs to ensure delivery on all networks
                    ...
                    _uiState.value = UiState.Error("Channel Error")
                }
            }
            return
        }
        ...
        if (contact != null) {
            ...
            // Note: SavedCode ignored for encryption now, but kept for handshake compatibility
            if (contact.savedCode.isNotEmpty()) {
                ...
                _uiState.value = UiState.Error("No IP Saved")
            }
            return
        }
    ...
    // UPDATED FUNCTION FOR AUTO-LEARNING
    fun onReceptionStarted(from: String, ip: String) {
        ...
        // Auto-learn IP: If we hear them, update list immediately so we can reply
        if (ip.isNotEmpty() && !from.startsWith("group:", true)) {
            viewModelScope.launch {
                ...
                if (existing != null) {
                    ...
                    nearbyUsers.add(Contact(from, ip, false))
                }
            }
        }
    }
    ...
    fun toggleBroadcastMode() {
        ...
        if (isBroadcastMode) {
            ...
            setTarget(repository.getTargetUser())
        }
    }

    fun toggleSpeaker(service: VoiceService?) {
        isSpeakerEnabled = !isSpeakerEnabled
        service?.toggleSpeaker(isSpeakerEnabled)
    }
    ...
    fun generateQr(content: String): Bitmap? {
        try {
            ...
            Log.e("WalkieViewModel", "QR generation failed", e)
        }
        return null
    }
    ...
    fun resetPairingCode(myName: String) {
        viewModelScope.launch {
            ...
            try {
                ...
                Log.e("WalkieViewModel", "Pairing code reset failed", e)
            }
        }
    }
}